...
| dimension cards (April Fools mode) | mode-off identity: composed bridge source === input source, byte for byte; renderer emits 0 card DOM/listeners/CSS; 7/7 e2e incl. park mid-reply → race → wake (history spliced) → fold → toggle off/on. |
