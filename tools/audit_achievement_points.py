#!/usr/bin/env python3
"""Fail-closed audit for local and synced workshop achievements."""
...
SCHEMA = "aibast-achievement-points-audit/1.0"
LOCAL_TO_CLAIM = {
    "started": "started",
...
LEGACY_ACRONYM = "".join(("A", "G", "I"))
LEGACY_PHRASE = "Agent " + "Growth"
...
def audit_point_contract(failures: Failures) -> None:
    local_points = scaffold_solution_journey.ACHIEVEMENT_POINTS
    server_points = build_metrics.ACHIEVEMENT_POINTS
    if tuple(LOCAL_TO_CLAIM.values()) != build_metrics.ACHIEVEMENT_ORDER:
...
def audit_generated_workshops(
    ...
    required_quest = (
        scaffold_solution_journey.ACHIEVEMENT_PROFILE_KEY,
        ...
        "earnedAchievementSyncIds(achievements)",
    ...
    for row in catalog:
        ...
        if f'const ACHIEVEMENT_WORKSHOP_SLUG = "{slug}";' not in quest:
            failures.add(f"{label}/quest.html: wrong achievement workshop slug")
        if f"const ACHIEVEMENT_CANONICAL_AGENT = {json.dumps(agent)};" not in quest:
            failures.add(f"{label}/quest.html: wrong canonical achievement agent")
        for claim_id in build_metrics.ACHIEVEMENT_ORDER:
            if f'claimId: "{claim_id}"' not in quest:
                failures.add(
                    f"{label}/quest.html: missing canonical claim ID {claim_id}"
                )
        if "aibast-obsolete-achievement" in quest or "aibast-obsolete-achievement" in manual:
            failures.add(f"{label}: obsolete achievement schema remains")
        for token in (
            "Withheld checkpoint",
            "reshoot required",
            "not approved for learner display",
        ):
            if token in quest or token in manual:
                failures.add(
                    f"{label}: internal visual QA language leaked to learners ({token})"
                )
        for document_name, document in (
            ...
            if (
                re.search(
                    rf"\b{re.escape(LEGACY_ACRONYM)}\b",
                    document,
                )
                or LEGACY_PHRASE in document
            ):
                failures.add(
                    f"{label}/{document_name}: legacy achievement branding remains"
                )
...
def audit_public_pages(root: Path, failures: Failures) -> None:
    ...
    for token in ("XMLHttpRequest", "sendBeacon", "aibast-obsolete-achievement"):
    ...
    if (
        ...
        or LEGACY_PHRASE in achievements
    ):
        failures.add("achievements.html: legacy achievement branding remains")

    for token in (
        'id="achievements-points"',
        'id="achievements-leaderboard"',
        'id="achievements-workshop-table"',
        'id="achievements-rollup-table"',
        "M.achievements || {}",
        "self-reported",
        "not independently",
    ):
...
def audit_parser(
    ...
    if not catalog:
        return
    workshop = catalog[0]
    achievement_list = ", ".join(build_metrics.ACHIEVEMENT_ORDER)
    body = (
        f"{build_metrics.ACHIEVEMENT_PROGRESS_MARKER}\n"
        "## Workshop achievement progress\n\n"
        f"- Schema: `{build_metrics.ACHIEVEMENT_PROGRESS_SCHEMA}`\n"
        f"- Workshop: `{workshop['slug']}`\n"
        f"- Agent: `{workshop['catalog_key']}`\n"
        f"- Achievements: {achievement_list}\n"
        "- Source: https://example.test/quest.html\n"
    )
    claim = build_metrics.parse_achievement_claim(body, catalog)
    if not claim or claim.get("achievements") != list(
        build_metrics.ACHIEVEMENT_ORDER
    ):
        failures.add("strict achievement parser rejected the canonical claim")
        return
    if build_metrics.parse_achievement_claim(body + "- Points: 999999\n", catalog):
        failures.add("strict achievement parser accepted body-supplied points")
    grouped = build_metrics.group_achievement_progress(
...
def audit_snapshot(
    ...
    achievements = snapshot.get("achievements") if isinstance(snapshot, dict) else None
    if not isinstance(achievements, dict) or achievements.get("schema") != "aibast-achievements/2.0":
