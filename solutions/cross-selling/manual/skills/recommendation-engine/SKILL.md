---
name: cross-selling-recommendation-engine
description: Draft prioritized product recommendations for CUST-001 without sending outreach.
---

# Cross-Selling Opportunities Agent — Recommendation Engine
...
## Procedure
1. Confirm that the request matches `recommendation_engine`.
...
4. Produce the exact fixed-snapshot evidence with the required `Prioritized Recommendations`, `Draft Engagement Plan`, `Evidence boundary` anchors.
...
## Locked demo prompt
Draft prioritized product recommendations and a reviewable engagement plan for synthetic customer CUST-001 without sending outreach.

## Expected evidence marker
The response must include `Prioritized Recommendations`, `Draft Engagement Plan`, `Evidence boundary` and preserve the explicit synthetic evidence boundary.
