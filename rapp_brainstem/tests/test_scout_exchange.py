...
def test_squad_restore_refuses_final_symlink(tmp_path):
    if not hasattr(Path, "symlink_to"):
        pytest.skip("symlinks are unavailable")
    source = exchange._source_record(b"replacement", "application/octet-stream")
    envelope = {
        "schema": exchange.SCHEMA,
        "artifact": {
            "kind": "scout-squad",
            "name": "bad",
            "files": [{"path": "linked.txt", **source}],
        },
        "mapping": {},
        "protocol": exchange.PROTOCOL,
    }
    ...
    link = output / "linked.txt"
    try:
        link.symlink_to(target)
    except OSError:
        pytest.skip("creating symlinks requires additional OS privileges")

    with pytest.raises(exchange.ExchangeError):
        exchange.skill_to_squad(skill, output)
