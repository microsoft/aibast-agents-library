...
description: Set up and operate an isolated RAPP Brainstem workspace from Microsoft Scout, with live static chat, RAPP/1 discovery, reversible capability exchange, local proof, and guarded Copilot Studio promotion.
...
- Display the repository's static `index.html` in Scout's center workspace pane.
- The static page talks to the hidden localhost Brainstem server through the
  generated ignored config. Do not show a `file://` page that remains offline.
- Never expose or commit `.brainstem_secret`, `preview-config.js`, Copilot
...
`brainstem_dir` to resolve to this workspace's `rapp_brainstem` directory.
When port 7071 belongs to another Brainstem, the controller selects the next
free port and writes that choice into the ignored preview config.
...
If authentication is required, keep the static chat visible and let the user
complete Brainstem's GitHub device-code flow. Do not claim success until health
is green.

## 5. Open live static chat in Scout

Open `rapp_brainstem/index.html` in the Scout workspace preview. The page must
show `connected`. It is the real repository file, not a copied UI and not an
external browser tab.

The ignored file
`agents/experimental/scout/preview-config.js` carries the active localhost URL
and per-install secret. Treat it as private runtime state.
...
2. Have Brainstem exercise the same capability through the visible static chat
   or `POST /chat`.
