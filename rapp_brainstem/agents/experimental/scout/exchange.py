...
def _write_no_follow(path: Path, data: bytes) -> None:
    if path.is_symlink():
        raise ExchangeError("refusing to overwrite a symlink")
    flags = os.O_WRONLY | os.O_CREAT | os.O_TRUNC
    flags |= getattr(os, "O_NOFOLLOW", 0)
    try:
        descriptor = os.open(path, flags, 0o600)
    except OSError as exc:
        raise ExchangeError(f"could not safely create squad output: {exc}") from exc
...
def skill_to_squad(source: Path, destination: Path) -> None:
    ...
    if destination.is_symlink():
        raise ExchangeError("squad destination must not be a symlink")
    ...
    for record in files:
        ...
        for part in relative.parts:
            ...
            if current.is_symlink():
                raise ExchangeError("squad output path contains a symlink")
        ...
        _write_no_follow(output, _decode_source(source_record))
