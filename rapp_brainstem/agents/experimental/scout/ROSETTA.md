...
3. Scout starts the hidden local server with `brainstem-workspace.ps1`.
4. The user opens the repository's static `index.html` in Scout's center pane.
5. The static page authenticates to the local server through the generated,
   ignored `preview-config.js`.
6. Scout and Brainstem build and test against the same workspace in real time.
7. Scout promotes the exact locally tested artifact to a Copilot Studio draft.
8. Publishing or sending remains an explicit user-approved action.
