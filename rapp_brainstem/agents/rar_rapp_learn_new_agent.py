...
v2.1: adds swarm generation, RAR registry compatibility, failed-library-search
handoff, generated self-tests, and the VS Code Brain Surgeon repair path.
...
__manifest__ = {
    ...
    "description": "Creates, self-tests, saves, and hot-loads RAPP agents or swarms from natural-language descriptions and unmet library searches. Includes a VS Code repair handoff.",
    "author": "RAPP",
    "tags": ["meta", "generator", "scaffolding", "learn", "swarm", "testing", "search-fallback"],
    "category": "core",
...
__manifest__ = {{
    ...
    "description": {agent_description_literal},
...
if __name__ == "__main__":
    a = {class_name}()
    output = a.perform(query="test")
    parsed = json.loads(output)
    assert isinstance(parsed, dict)
    assert parsed.get("status") in {{"success", "ok", "error"}}
    print(output)
'''
...
'''
    ...
    def __init__(self):
        ...
        self.metadata = {
            ...
            "description": (
                ...
                "custom agent, build an agent swarm, or turn an unmet library search into "
                "a working agent. Every create result includes a local test-and-repair path."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    def _create_agent(self, description, name='', write=True, **kwargs):
        ...
        loaded = hot_load_result.get("success", False)

        result = {
            "status": "success" if loaded else "error",
            "action": "create",
            "message": (
                f"Created, smoke-tested, and loaded agent '{name}'"
                if loaded
                else f"Created agent '{name}', but its smoke test failed"
            ),
            "agent_name": name,
            "filename": file_name,
            ...
            "hint": (
                f"Agent saved to agents/{file_name}. "
                + ("The hot-load smoke test passed. " if loaded else "The hot-load smoke test did not pass. ")
                + "To submit to RAR, re-run with action='submit'."
            ),
            "need_help": (
                "If the agent is not doing what you expect: export this Brainstem "
                "conversation transcript, click the VS Code button to open Copilot Chat "
                "(Brain Surgeon), drag the transcript into that window, and say: "
                f"'Test and fix agents/{file_name} against my local Brainstem. Preserve "
                "the manifest and tool schema, run the failing prompt, fix the root cause, "
                "and verify it through localhost:7071 before you finish.'"
            ),
            "repair_prompt": (
                f"Test and fix agents/{file_name} against my local Brainstem. "
                "Use the attached Brainstem transcript as the reproduction. Preserve "
                "the manifest and tool schema, run the failing prompt, fix the root "
                "cause, and verify it through localhost:7071 before you finish."
            ),
        }
        ...
        if not hot_load_result.get("success"):
            ...
            result["message"] += (
                " The first hot-load test failed; use the need_help transcript handoff."
            )
    ...
    def _create_swarm(self, description, swarm_name='', write=True, **kwargs):
        ...
        namespace = self._sanitize_namespace(kwargs.get('namespace', ''))
        ...
        for role in sub_roles:
            ...
            sub_code = self.SWARM_SUB_TEMPLATE.format(
                ...
                agent_name=sub_name,
                agent_description_literal=repr(sub_desc),
                author_literal=repr(namespace),
        ...
        orch_code = self.SWARM_ORCH_TEMPLATE.format(
            ...
            agent_description_literal=repr(safe_desc),
            author_literal=repr(namespace),
    ...
    def _sanitize_namespace(self, namespace):
        value = (namespace or 'rapp').lstrip('@')
        if not re.fullmatch(r'[A-Za-z0-9](?:[A-Za-z0-9_-]{0,38})', value):
            return 'rapp'
        return value
    ...
    def _generate_perform_body(self, description):
        try:
            prompt = (
                ...
                f"- Validate required input and return explicit error JSON\n"
                f"- Keep it simple, deterministic, and functional\n"
                f"- Do not swallow exceptions or return success-shaped fallbacks\n"
    ...
    def _smoke_test_agent(self, agent_class, module, timeout=5):
        outcome = queue.Queue(maxsize=1)
        example_args = (
            getattr(module, "__manifest__", {})
            .get("example_call", {})
            .get("args", {})
        )
        ...
        def run():
            try:
                ...
                result = instance.perform(**example_args)
                if not isinstance(result, str):
                    raise TypeError("perform() must return a string")
                parsed = json.loads(result)
                if not isinstance(parsed, dict) or "status" not in parsed:
                    raise ValueError("perform() must return JSON with status")
                outcome.put({"success": True, "result": parsed})
            except Exception as exc:
                outcome.put({"success": False, "error": str(exc)})

        thread = threading.Thread(target=run, daemon=True)
        thread.start()
        thread.join(timeout)
        if thread.is_alive():
            return {"success": False, "error": "Smoke test timed out"}
    ...
    def _hot_load_agent(self, file_path, class_name):
        try:
            ...
            smoke_test = self._smoke_test_agent(agent_class, module)
            if not smoke_test.get("success"):
                return {
                    "success": False,
                    "error": f"Smoke test failed: {smoke_test.get('error', 'unknown error')}",
                }
            ...
            result = {
                "success": True,
                "class": class_name,
                "smoke_test": smoke_test.get("result"),
            }
            if missing_deps:
